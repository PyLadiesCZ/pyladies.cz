...
\startsection{Následující projekty procvičí orientaci v chybových hláškách. Doporučuji je udělat.}
...
\item \emph{\small Zkus se nedívat na programy ze srazu.}
    \\Napiš program, který spočítá povrch a objem krychle o straně \SI{2852}{cm}.
    \\Abys nemusela tolik hledat v učebnici (vlastně Wikipedii): povrch $S = 6a^2$, objem $V = a^3$
...
\item Změň program tak, aby stranu/poloměr mohl uživatel zadat.
    \\\emph{\small Tady už se na materiály ze srazu klidně podívej.}
...
\item Ulož si následující program jako \texttt{pokus.py} a spusť ho několikrát za sebou. Co dělá?
    ...
    \\\emph{\small
        Aby všechno fungovalo správně, \emph{nesmíš} mít v aktuálním
        adresáři soubor jménem \texttt{random.py}.
    }

\item Zkombinuj program z předchozího projektu s programem kámen-nůžky-papír a nastav \verb+tah_pocitace+ na:
    \begin{enumerate}
        \item \verb+'kámen'+, pokud je \verb+cislo+ 0,
...
\startsection{Tyhle projekty kombinují opakování a nové informace.
...
\item Projdi si v materiálech bonusovou sekci „\emph{Nebo} anebo \emph{a}”:
    \\\url{http://naucse.python.cz/lessons/beginners/and-or/}
